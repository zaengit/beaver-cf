...
[![npm](https://img.shields.io/npm/v/@zaengit/beaver-cf?logo=npm)](https://www.npmjs.com/package/@zaengit/beaver-cf)
...
The package is published to npmjs as `@zaengit/beaver-cf` when a `v*` Git tag
is pushed. The tag must match the package version; for example, version `0.1.0`
is released with tag `v0.1.0`.

Configure a GitHub Actions environment named `npm` and add an `NPM_TOKEN`
secret with permission to publish this package. The release workflow runs
`npm ci`, builds and tests the package, validates its publishable contents,
publishes it as a public scoped package with npm provenance, then verifies the
new version is visible from `registry.npmjs.org`.
